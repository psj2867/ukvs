...
import { getLogger, HandlerList, nowMs, timeoutPromise } from "./util";
...
export class SyncHandler {
  ...
  init() {
    this.memberManager.receiver["sync_req"] = async (
      ...
      let resData = await member.extra.waitNext("sync_res");
      sync_logger.debug(
  ...
  async sync(member, gossip = false) {
    ...
    let resReqData = await member.extra.waitNext("sync_res&req");
    sync_logger.debug(
...
export class HeartbeatHandler {
  ...
  init() {
    this.memberManager.receiver["heartbeat"] = (d, member) => {
      ...
      heartbeat_logger.debug(`delay with ${member.userUUID} = ${delay}ms`);
      member.extra.alive = true;
      if (orgAlive != member.extra.alive)
        this._onMemberHandlers.handle(member.userUUID, member, "changeAlive");
    };
    return this;
  }
  ...
  async heartbeatOnce(member, timeout) {
    ...
    let response = await timeoutPromise(
      member.extra.waitNext("heartbeat_ret", true),
      timeout
    );
...
export class CopySpecialDataHandler {
  ...
  async requestCopySpecialData(member) {
    ...
    let d = await member.extra.waitNext("copySpecial_ret", true);
    this.memberManager.setState("node");
    mergeData(this.storageManager, d);
  }
}
const data_logger = getLogger("data_message");
const special_logger = getLogger("special_message");
export class SetHandler {
  ...
  getTTL() {
    ...
    return Math.max(minTTL, Math.min(maxTTL, maxDis)) + 1;
  }
}

export class DiconnectRequestHandler {
  /**
   * @param {MemberManager} memberManager
   * @param {StorageManager} storageManager
   */
  ...
  init() {
    this.memberManager.receiver["disconnect"] = (d, conn) => {
      this.memberManager.disconnect(conn, `recv:req:${d.data.comment}`);
    };
    return this;
  }
  /**
  ...
  async requestDisconnect(member, comment) {
    ...
    this.memberManager.disconnect(member, `send:req:${comment}`);
  }
}
