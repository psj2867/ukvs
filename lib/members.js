...
import { DiconnectRequestHandler, HeartbeatHandler } from "./handlers";
...
export class MemberManager {
  ...
  constructor(userUUID = null, members, config = {}) {
    ...
    this.connectionManager.onConnection(async (c) => {
      c.extra.open = false;
      this.setReceiver(c);
    });
  ...
  async init() {
    if (this.storage == null) {
      throw "init after call setStorage";
    }
    ...
    this.disconnectRequestHandler = new DiconnectRequestHandler(
      this,
      this.storage.manager
    ).init();
  ...
  disconnect(member, comment) {
    logger.debug(
      `disconnect with ${member.userUUID} by ${comment} state=${
  ...
  isInConn(
    ...
    let conns = this.getConnMembers(userUUID, onlyAlive, open, state);
    if (inlucdeMe) conns.push({ userUUID: this.myUserUUID });
    return conns.filter((e) => e.userUUID == userUUID).length > 0;
  }
  isNotInConn(
    userUUID,
    onlyAlive = false,
    ...
    return !this.isInConn(userUUID, onlyAlive, open, state, inlucdeMe);
  }
...
export class Members {
  ...
  async join(remoteId) {
    ...
    if (remoteId == this.myUserUUID) throw "reject: join with myself";
    if (
      this.getConnMembers(false, false)
        .map((e) => e.userUUID)
        .indexOf(remoteId) >= 0
    )
      throw "reject: join with dup";
