...
export class MemberManager {
  ...
  constructor(userUUID = null, members, config = {}) {
    ...
    this.config = {
      ...
      disCacheTime: 500,
      rtc_config: null,
      peerjs_server_host: "n1.psj2867.com",
  ...
  async init() {
    if (this.storage == null) {
      throw new UkvsError("init after call setStorage");
    }
...
export class Members {
  ...
  async _join(remoteId) {
    try {
      ...
      throw new UkvsError(error);
    }
  }
  async join(remoteId) {
    logger.debug(`try join with ${remoteId}`);
    if (remoteId == this.myUserUUID)
      throw new UkvsError("reject: join with myself");
    if (this.manager.isInConn(remoteId))
      throw new UkvsError("reject: join with dup");
