...
export class Reconnector {
  ...
  async reconnect() {
    ...
    await this.reconnectForMismatchInfo();
  ...
  async disconnectDup() {
    ...
    connList = connList
      .sort((a, b) => a.connectedTime - b.connectedTime)
      .reverse();
  ...
  disconnectN(n) {
    ...
    let shuffled = this.memberManager
      ...
      .sort((a, b) => b.connectedTime - a.connectedTime)
      .map((e, idx) => {
        e.idx = idx;
        return e;
      })
    ...
    for (const member of selected) {
      this.memberManager.disconnect(member, "limit");
    }
  }
